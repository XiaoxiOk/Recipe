// pages/msg-comment/msg-comment.js
...
Page({
     ...
     */
    onShow() {
        console.log('show-comment/reply-msg')
        this.reFreshData()
    },
    ...
    onReachBottom() {
        ...
        //已加载完毕时不再去请求
        if (self.data.cmtMsgList.length != 0 && self.data.cmtMsgList.length < self.data.page.total) {
    ...
    getCmtMsgList() {
        ...
        console.log('加载评论/回复消息列表')
        var params = {
            // 请求连接
            url: '/message/getCommentMsgList?userId=' + self.data.myId,
            method: "POST",
            // 请求所需要的的参数
            data: self.data.page,
            callBack: (resData) => {
                console.log('comt/reply-resData', resData)
                self.setData({
                    page: {
                ...
                if (dataRecords) {
                    if (dataRecords.length >= 1) {
                        ...
                        self.setData({
                            cmtMsgList: self.data.cmtMsgList.concat(msgList),
                        });
                    }
                }
                wx.hideLoading();
    ...
    handleMsgData(rawList) {
        for (var i = 0; i < rawList.length; i++) {
            ...
            switch (rawList[i].type) {
                case msgType.comment_recipe: {
                    textTip = "评论了你的菜谱";
                    contentDetail = contentJson.commentContent
                    rawList[i].showRecipeImage = contentJson.showImage
                    break;
                }
                case msgType.comment_user_share: {
                    textTip = "评论了你的动态";
                    contentDetail = contentJson.commentContent
                    rawList[i].showOrginContent = contentJson.userShareContent
                    break;
                }
                case msgType.reply_you: {
                    textTip = "回复了你"; 
                    contentDetail = contentJson.replyContent
                    rawList[i].showOrginContent = contentJson.commentContent
                    break;
                }
                default: ""
            }
    ...
    goToOtherUserPage(e) {
        ...
        let userId = this.data.cmtMsgList[index].userId
        if (userId > 0) {
            wx.navigateTo({
