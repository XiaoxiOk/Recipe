      ...
      if (responseData.status === 501) {
        wx.navigateTo({
         ...
         if (responseData.status === 300) {
            ...
            wx.showToast({
              title: responseData.msg ? responseData.msg : 'Error',
              icon: 'none'
            })
            return
          }
...
//通过status获取token,并保存到缓存
var getToken = function() {
  wx.login({
    success: res => {
      // 发送 res.status 到后台换取 openId, sessionKey, unionId
      request({
        login: true,
